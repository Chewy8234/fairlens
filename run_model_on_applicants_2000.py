"""
Loads test_applicants_2000.csv, runs hiring_model.pkl, and reports results
...
df = pd.read_csv("test_applicants_2000.csv")
df = df.map(lambda x: x.strip() if isinstance(x, str) else x)
print(f"Loaded {len(df)} applicants from test_applicants_2000.csv\n")
...
df[["Name","Gender","Ethnicity","Job","GPA","Predicted_Fit","Hired"]].to_csv(
    "predictions_2000.csv", index=False)

# ── Console report ────────────────────────────────────────────────────────────
W = 62
print("=" * W)
print("   HIRING MODEL RESULTS — test_applicants_2000.csv (n=2000)")
print("=" * W)
...
print(f"  Impact ratio (F/M): {impact_ratio:.3f}")
...
print(f"\n  Full predictions saved → predictions_2000.csv")
...
fig.suptitle("AI Hiring Model — Gender Bias Analysis (2000 Applicants)",
...
width  = 0.35
for i, gen in enumerate(fit_pct.index.tolist()):
    offset = (i - 0.5) * width
    ax4.bar(x + offset, fit_pct.loc[gen, fit_order], width,
...
plt.savefig("applicant_bias_report_2000.png", dpi=150, bbox_inches="tight")
print("  Chart saved → applicant_bias_report_2000.png")
