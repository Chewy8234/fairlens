"""
Generates test_applicants_500.csv, runs hiring_model.pkl on it,
...
rows = [make_person(rng, "male") for _ in range(250)] + \
       [make_person(rng, "female") for _ in range(250)]
df   = pd.DataFrame(rows).sample(frac=1, random_state=42).reset_index(drop=True)
df.to_csv("test_applicants_500.csv", index=False)
print(f"\n  Generated test_applicants_500.csv  ({len(df)} rows)")
...
report = f"""
{SEP}
   AI HIRING BIAS REPORT — GENDER ANALYSIS (500 CANDIDATES)
{SEP}
...
print(report)
with open("results_summary_500.txt", "w") as fh:
    fh.write(report)
print("  Report saved to: results_summary_500.txt\n")

# ── Charts ────────────────────────────────────────────────────────────────────
overall = pd.DataFrame({"Male": [m_rate], "Female": [f_rate]}, index=["All Candidates"])
bar_chart(overall,
          title="AI Hiring Model — Hire Rate by Gender (500 Candidates)",
          xlabel="", filename="bias_gender_chart_500.png", figsize=(7, 6), bar_width=0.2)

field_pivot = hire_rate_pivot(df, "Job").reindex(sorted(df["Job"].unique()))
bar_chart(field_pivot,
          title="Gender Bias by Job Field — 500 Candidates\n(Hire Rate Comparison — Male vs Female)",
          xlabel="Job Field", filename="bias_by_field_500.png",
...
bar_chart(tier_pivot,
          title="Gender Bias by Education Tier — 500 Candidates\n(Hire Rate Comparison — Male vs Female)",
          xlabel="Education Tier", filename="bias_by_university_tier_500.png", figsize=(10, 7))
